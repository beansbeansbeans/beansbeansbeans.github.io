...
void main() {
  vec4 upperLeftRaw = texture2D(textureUy, vec2(gl_FragCoord.x - 1., gl_FragCoord.y + 1.) / resolution.xy);
  float upperLeft = (upperLeftRaw.x * upperLeftRaw.x + upperLeftRaw.y * upperLeftRaw.y + upperLeftRaw.z * upperLeftRaw.z) / 3.;

  vec4 aboveRaw = texture2D(textureUy, vec2(gl_FragCoord.x, gl_FragCoord.y + 1.) / resolution.xy);
  float above = (aboveRaw.x * aboveRaw.x + aboveRaw.y * aboveRaw.y + aboveRaw.z * aboveRaw.z) / 3.;

  vec4 upperRightRaw = texture2D(textureUy, vec2(gl_FragCoord.x + 1., gl_FragCoord.y + 1.) / resolution.xy);
  float upperRight = (upperRightRaw.x * upperRightRaw.x + upperRightRaw.y * upperRightRaw.y + upperRightRaw.z * upperRightRaw.z) / 3.;

  vec4 leftRaw = texture2D(textureUy, vec2(gl_FragCoord.x - 1., gl_FragCoord.y) / resolution.xy);
  float left = (leftRaw.x * leftRaw.x + leftRaw.y * leftRaw.y + leftRaw.z * leftRaw.z) / 3.;

  vec4 middleRaw = texture2D(textureUy, vec2(gl_FragCoord.x, gl_FragCoord.y) / resolution.xy);
  float middle = (middleRaw.x * middleRaw.x + middleRaw.y * middleRaw.y + middleRaw.z * middleRaw.z) / 3.;

  vec4 rightRaw = texture2D(textureUy, vec2(gl_FragCoord.x + 1., gl_FragCoord.y) / resolution.xy);
  float right = (rightRaw.x * rightRaw.x + rightRaw.y * rightRaw.y + rightRaw.z * rightRaw.z) / 3.;

  vec4 lowerLeftRaw = texture2D(textureUy, vec2(gl_FragCoord.x - 1., gl_FragCoord.y - 1.) / resolution.xy);
  float lowerLeft = (lowerLeftRaw.x * lowerLeftRaw.x + lowerLeftRaw.y * lowerLeftRaw.y + lowerLeftRaw.z * lowerLeftRaw.z) / 3.;

  vec4 belowRaw = texture2D(textureUy, vec2(gl_FragCoord.x, gl_FragCoord.y - 1.) / resolution.xy);
  float below = (belowRaw.x * belowRaw.x + belowRaw.y * belowRaw.y + belowRaw.z * belowRaw.z) / 3.;

  vec4 lowerRightRaw = texture2D(textureUy, vec2(gl_FragCoord.x + 1., gl_FragCoord.y - 1.) / resolution.xy);
